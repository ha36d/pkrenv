...
function pkrenv-min-required() {
  local path="${1:-${PKRENV_DIR:-.}}";

  local versions="$( echo $(cat ${path}/{*.hcl,*.json} 2>/dev/null | grep -Eh '^\s*[^#]*\s*required_version') | grep -o '[~=!<>]\{0,2\}\s*\([0-9]\+\.\?\)\{2,3\}\(-[a-z]\+[0-9]\+\)\?')";
  ...
  if [[ "${versions}" =~ ([~=!<>]{0,2}[[:blank:]]*)([0-9]+[0-9.]+)[^0-9]*(-[a-z]+[0-9]+)? ]]; then
    ...
    if [[ "${qualifier}" =~ ^!= ]]; then
      ...
      local min_required_file="$(grep -Hn required_version ${path}/{*.hcl,*.json} 2>/dev/null | xargs)";

      # Probably not an advisable way to choose a packer version,
      # but this is the way this functionality works in packer:
